...
function assignCellFromProbability(baseIndex, x, y, z) {
  ...
  if (u < 8870) return 8;
  if (u < 9870) return 9;
  if (u < 10120) return 10;
  if (u < 11120) return 5;
  if (u < 11370) return 6;
  if (u < 13620) return 120;
  if (u < 23620) return 4;
  if (u < 24120) return 200; // mirror 0.05%
...
function rotate(pos, deg) {
  ...
  for (let x = 0; x < 7; x += 1) {
    for (let y = 0; y < 7; y += 1) {
      for (let z = 0; z < 7; z += 1) {
        const value = temp[x][y][z];
        if (!value) continue;
        const bx = x + state.blockpos[0];
        const by = y + state.blockpos[1];
        const bz = z + state.blockpos[2];
        if (bx < 0 || bx > 6 || by < 0 || by > 6 || bz < 0 || bz > 25) {
          return 1;
        }
        const blockValue = state.blk[bx][by][bz];
...
function processLine(cells, z, coords) {
  ...
  for (const [x, y] of coords) {
    ...
    if (code === 116) { cells.tline -= 2; state.blk[x][y][z] = 256; }
    ...
    else if (code === 17) { state.bombnext += 3; state.blk[x][y][z] = 256; }
    ...
    else if (code === 22) { state.monoonly = 0; state.simplify2 = 0; state.pentaForce += 6; state.blk[x][y][z] = 256; }
      ...
      // 거울상: mirror board horizontally (flip x-axis)
      state.blk[x][y][z] = 256;
      for (let mz = 0; mz < 26; mz++) {
        for (let my = 0; my < 7; my++) {
          for (let mx = 0; mx < 3; mx++) {
            const tmp = state.blk[mx][my][mz];
            state.blk[mx][my][mz] = state.blk[6 - mx][my][mz];
            state.blk[6 - mx][my][mz] = tmp;
          }
        }
      }
    }
      ...
      for (let x2 = 0; x2 < 7; x2 += 1) {
        for (let y2 = 0; y2 < 7; y2 += 1) {
          for (let z2 = 0; z2 < 26; z2 += 1) {
            if ((state.blk[x2][y2][z2] & 255) !== 0 && randInt(10) === 0) {
              state.blk[x2][y2][z2] = (state.blk[x2][y2][z2] & 256) + 120;
            }
          }
        }
      }
...
function clickbutton(x, y) {
  ...
  if ((x - 0.5) * (x - 0.5) + (y + 0.525) * (y + 0.675) < 0.03) {
    state.floorz = (state.floorz + 1) % 7;
    return 0;
  }
  if ((x - 0.5) * (x - 0.5) + (y + 1.125) * (y + 1.125) < 0.09) {
  ...
  if (-0.285 < x && x < -0.095 && -1.295 < y && y < -1.105) { rotate(0, -1); return 0; }
  if (-0.285 < x && x < -0.095 && -1.085 < y && y < -0.895) { rotate(0, 1); return 0; }
  if (-0.495 < x && x < -0.305 && -1.295 < y && y < -1.105) { rotate(1, -1); return 0; }
  if (-0.495 < x && x < -0.305 && -1.085 < y && y < -0.895) { rotate(1, 1); return 0; }
  if (-0.705 < x && x < -0.505 && -1.295 < y && y < -1.105) { rotate(2, -1); return 0; }
  if (-0.705 < x && x < -0.505 && -1.085 < y && y < -0.895) { rotate(2, 1); return 0; }
...
function handleTouches() {
  ...
  if (t0.flag === 1) {
    ...
    if (state.otp + 80 < now()) {
      state.otp = now();
      if (state.ul === 0) clickbutton(t0.x, t0.y);
      else state.ul -= 1;
    }
...
function drawSpecialPic(pic, x, y, z, t, color, val) {
  ...
  if (pic === 81) {
    // Front face: top box + two bottom boxes
