...
function rotateCellsCW(piece) {
  // Save original bounding box min corner to prevent drift
  let minR = Infinity, minC = Infinity;
  for (const [r, c] of piece.cells) { if (r < minR) minR = r; if (c < minC) minC = c; }
  // Rotate each cell CW around centroid: (r,c) -> (-c, r)
  for (let i = 0; i < piece.cells.length; i++) {
    const [r, c] = piece.cells[i];
    piece.cells[i] = [-c, r];
  }
  // Restore bounding box min corner to original position
  let newMinR = Infinity, newMinC = Infinity;
  for (const [r, c] of piece.cells) { if (r < newMinR) newMinR = r; if (c < newMinC) newMinC = c; }
  const dr = minR - newMinR, dc = minC - newMinC;
  for (let i = 0; i < piece.cells.length; i++) {
...
function rotateCellsCCW(piece) {
  let minR = Infinity, minC = Infinity;
  for (const [r, c] of piece.cells) { if (r < minR) minR = r; if (c < minC) minC = c; }
  // Rotate each cell CCW around centroid: (r,c) -> (c, -r)
  for (let i = 0; i < piece.cells.length; i++) {
    const [r, c] = piece.cells[i];
    piece.cells[i] = [c, -r];
  }
...
function processLine(row) {
  ...
  for (let c = 0; c < BOARD_W; c++) {
    ...
    if (code === 116) { tline -= 2; state.board[row][c] = 256; }
    ...
    } else if (code === 104) { state.simplify2 = 0; state.pentaForce = 0; state.monoonly += 10; state.board[row][c] = 256; }
      ...
      // 거울상: mirror board horizontally (flip columns)
      state.board[row][c] = 256;
      for (let mr = 0; mr < BOARD_H; mr++) {
        for (let mc = 0; mc < Math.floor(BOARD_W / 2); mc++) {
          const tmp = state.board[mr][mc];
          state.board[mr][mc] = state.board[mr][BOARD_W - 1 - mc];
          state.board[mr][BOARD_W - 1 - mc] = tmp;
        }
      }
    }
    else if (code === 11) {
