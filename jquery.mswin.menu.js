(function ($, undefined) {
    ...
    $.widget("mswin.menubar", $.ui.menu, {
        ...
        _create: function () {
            ...
            // 문서 전체에서 메뉴 바깥 클릭 시 닫기
            var _this = this;
            this.document.on("pointerdown.menubar", function (event) {
                if (!$.contains(_this.element[0], event.target)) {
                    _this.element.find(".ui-state-open").removeClass("ui-state-open");
                    _this.collapseAll(event);
                }
            });
        },
